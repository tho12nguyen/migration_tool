...
def build_mappings(sheets) -> tuple:
    ...
    for row in sheets['schema'].to_numpy()[2:]:
        schema_dict.setdefault(row[3].upper(), set()).add(row[5])
    for row in sheets['table'].to_numpy()[2:]:
        table_dict.setdefault(row[4].upper(), set()).add(row[7])
    for row in sheets['column'].to_numpy()[2:]:
        table, column, value = row[4], row[5], row[9]
        column_dict.setdefault(table.upper(), {}).setdefault(column.upper(), set()).add(value)
    for row in sheets['key'].to_numpy()[2:]:
        key_dict.setdefault(row[5].upper(), set()).add(row[8])
    schema_dict['金庫'] = {'schm_kinko'}
